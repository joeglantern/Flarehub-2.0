...
function toComparable(v: FieldResponseValue): string | number | boolean {
  ...
  switch (condition.operator) {
    ...
    case 'not_contains':
      return typeof comparable === 'string'
        && !comparable.toLowerCase().includes(String(condVal).toLowerCase())
    case 'greater_than':
      return Number(comparable) > Number(condVal)
    case 'less_than':
      return Number(comparable) < Number(condVal)
    default:
      return false
...
export function evaluateConditionalLogic(
  ...
  const met = logic.match === 'all' ? results.every(Boolean) : results.some(Boolean)
